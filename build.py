...
function onAudioEnded(audio){
    ...
    const idx=allAudio.indexOf(audio);
    if(idx>=0&&idx<allAudio.length-1){
        allAudio[idx+1].scrollIntoView({behavior:'smooth',block:'center'});
        allAudio[idx+1].play();
    }
}
...
function filterAndSort(){
    ...
    shown=0;document.getElementById("episodes").innerHTML='';
    document.getElementById("no-results").style.display='none';currentAudio=null;
    loadMore();
}
function loadMore(){
...
document.addEventListener('keydown',e=>{
    ...
    if(e.key==='ArrowLeft')window.scrollBy({top:-400,behavior:'smooth'});
...
function buildSourceFilter(){
    ...
    allEpisodes.forEach(ep=>{
        const f=ep.fuente.toLowerCase();
        if(f.includes('radio nacional'))s['rn']=(s['rn']||0)+1;
        else if(f.includes('anchor'))s['anchor']=(s['anchor']||0)+1;
        else if(f.includes('radiocut'))s['rc']=(s['rc']||0)+1;
        else if(f.includes('youtube'))s['yt']=(s['yt']||0)+1;
    });
    const sel=document.getElementById("source-filter");
    if(s['rn'])sel.innerHTML+='<option value="rn">Radio Nacional ('+s['rn']+')</option>';
    if(s['anchor'])sel.innerHTML+='<option value="anchor">Anchor.fm ('+s['anchor']+')</option>';
